...
void SddVioNodelet::imgCb(const sensor_msgs::ImageConstPtr& msg_0,
...
void SddVioNodelet::imuCb(const sensor_msgs::Imu::ConstPtr& msg)
{
...
void SddVioNodelet::loadParams(ros::NodeHandle &nh)
{
...
void SddVioNodelet::initRosIO(ros::NodeHandle &nh)
{
  ...
  sync_->registerCallback(boost::bind(&sdd_vio::SddVioNodelet::imgCb, this, _1, _2));
  ...
  imu_cache_->registerCallback(
      boost::bind(&sdd_vio::SddVioNodelet::imuCb, this, _1));
}

void SddVioNodelet::initVo(ros::NodeHandle &nh)
{
  vo_.reset(new sdd_vio::VoStereo(stereoCam_.get(), "sdd_vio_node", nh));
  vo_->start();
}

void SddVioNodelet::onInit(void)
{
  std::cout << "--------------------------" << std::endl;
...
PLUGINLIB_EXPORT_CLASS(sdd_vio::SddVioNodelet, nodelet::Nodelet);
